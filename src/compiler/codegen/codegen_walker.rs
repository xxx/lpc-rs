...
/// Whether a loop condition holds without a test: absent, or a non-zero
/// integer literal.
fn always_true(condition: Option<&ExpressionNode>) -> bool {
    match condition {
        None => true,
        Some(ExpressionNode::Int(IntNode { value, .. })) => *value != 0,
        Some(_) => false,
    }
}
...
/// The value register of `arg`, for a position semantic checks already
/// guarantee never receives a `ref` (`sizeof`'s argument, `call_other`'s
/// receiver and name).
fn expect_value(arg: Arg, span: Option<Span>) -> Result<RegisterVariant> {
    match arg {
        Arg::Value(r) => Ok(r),
        Arg::Ref(_) => Err(lpc_bug!(
            span,
...
impl CodegenWalker {
    ...
    async fn build_initializer(&mut self, init_globals: Ustr) -> Result<ProgramFunction> {
        ...
        for init in inits.into_iter().chain(std::iter::once(init_globals)) {
            let list = self.arg_list(vec![], None)?;
            push_instruction!(self, Instruction::Call(init, list), None);
        }
    ...
    fn choose_op_instruction(
        ...
        match node.op {
            ...
            BinaryOperation::EqEq => {
                Instruction::Cmp(Comparison::Eq, reg_left, reg_right, reg_result)
            }
            BinaryOperation::NotEq => {
                Instruction::Cmp(Comparison::Ne, reg_left, reg_right, reg_result)
            }
            BinaryOperation::Lt => {
                Instruction::Cmp(Comparison::Lt, reg_left, reg_right, reg_result)
            }
            BinaryOperation::Lte => {
                Instruction::Cmp(Comparison::Lte, reg_left, reg_right, reg_result)
            }
            BinaryOperation::Gt => {
                Instruction::Cmp(Comparison::Gt, reg_left, reg_right, reg_result)
            }
            BinaryOperation::Gte => {
                Instruction::Cmp(Comparison::Gte, reg_left, reg_right, reg_result)
            }
    ...
    /// a fresh entry per call site.
    fn arg_list(&mut self, args: Vec<Arg>, span: Option<Span>) -> Result<ArgList> {
        let Some(function) = self.function_stack.last_mut() else {
            return Err(lpc_bug!(
                span,
                "no open function to add an argument list to"
            ));
        };
        let Ok(index) = u16::try_from(function.arg_lists.len()) else {
            return Err(lpc_error!(
                span,
                "too many calls in one function (the limit is {})",
                usize::from(u16::MAX) + 1
            ));
        };
        function.arg_lists.push(args);
        Ok(ArgList(index))
    }
    ...
    /// Open an inverted loop: jump forward to its test, unless the condition
    /// always holds and the bottom test will be a bare jump back.
    fn emit_loop_entry(
        &mut self,
        ...
        self.emit_jump(test_label, span)
    }

    /// Close an inverted loop: jump back to `start_label` while the condition
    /// holds.
    async fn emit_loop_test(
        &mut self,
        condition: Option<&mut ExpressionNode>,
        start_label: &Label,
        span: Option<Span>,
    ) -> Result<()> {
        match condition {
            Some(condition) => {
                self.emit_condition(condition, JumpWhen::True, start_label)
                    .await
            }
            None => self.emit_jump(start_label, span),
        }
    }

    /// The address the next pushed instruction takes.
...
impl TreeWalker for CodegenWalker {
    ...
    async fn visit_binary_op(&mut self, node: &mut BinaryOpNode) -> Result<()> {
        ...
        match node.op {
            ...
            BinaryOperation::Compose => {
                ...
                let reg_right = self.current_result;
                let list =
                    self.arg_list(vec![Arg::Value(reg_left), Arg::Value(reg_right)], node.span)?;
                push_instruction!(
                    self,
                    Instruction::CallEfun(
                        u8::try_from(EFUN_PROTOTYPES.get_index_of("compose").unwrap())?,
                        list,
                    ),
                    node.span
                );
    ...
    async fn visit_call_root(&mut self, node: &mut CallNode) -> Result<()> {
        ...
        let instruction = {
            if let Some(receiver_result) = receiver_result {
                ...
                let receiver = expect_value(arg_results[0], node.span)?;
                let name_index = expect_value(arg_results[1], node.span)?;
    ...
    async fn visit_call_chain(&mut self, node: &mut CallNode) -> Result<()> {
        ...
        push_instruction!(self, Instruction::CallFp(fp_loc, list), node.span);
    ...
    async fn visit_for(&mut self, node: &mut ForNode) -> Result<()> {
        ...
        let test_label = self.new_label("for-test");
        let end_label = self.new_label("for-end");
        let continue_label = self.new_label("for-continue");
        let jump_target = JumpTarget::new(end_label.clone(), continue_label.clone());
        self.jump_targets.push(jump_target);

        self.emit_loop_entry(node.condition.as_ref(), &test_label, node.span)?;
        ...
        self.emit_loop_test(node.condition.as_mut(), &start_label, node.span)
            .await?;
    ...
    async fn visit_while(&mut self, node: &mut WhileNode) -> Result<()> {
        ...
        let test_label = self.new_label("while-test");
        let end_label = self.new_label("while-end");
        self.jump_targets
            .push(JumpTarget::new(end_label.clone(), test_label.clone()));

        self.emit_loop_entry(Some(&node.condition), &test_label, node.span)?;
        ...
        let test_addr = self.current_address();
        self.insert_label(test_label, test_addr);
        self.emit_loop_test(Some(&mut node.condition), &start_label, node.span)
            .await?;
...
mod tests {
    ...
    mod test_visit_call {
        ...
        async fn populates_the_instructions_for_local_calls() {
            ...
            let expected = vec![Call(
                ustr("local_function__v__/test/local.c__pb__"),
                ArgList(0),
            )];
        ...
        async fn populates_the_instructions_for_simul_efuns() {
            ...
            let expected = vec![CallSimulEfun(ustr("simul_efun"), ArgList(0))];
        ...
        async fn copies_non_void_call_results() {
            ...
            let expected = vec![
                Call(ustr("marfin__i__marfin.c__pb__"), ArgList(0)),
                Copy(
                    RegisterVariant::Local(Register(0)),
                    RegisterVariant::Local(Register(1)),
                ),
        ...
        async fn does_not_copy_void_call_results() {
            ...
            let expected = vec![Call(ustr("void_thing__v__void_thing.c__pb__"), ArgList(0))];
        ...
        async fn handles_ellipsis_functions() {
            ...
            let expected = vec![Call(ustr("my_func__v__my_func.c__pb_e__s"), ArgList(0))];
        ...
        async fn handles_chained_calls() {
            ...
            let expected = vec![
                ...
                CallFp(RegisterVariant::Local(Register(3)), ArgList(1)),
                Copy(
                    RegisterVariant::Local(Register(0)),
    ...
    mod test_constant_pool {
        ...
        async fn a_call_other_name_is_pooled() {
            ...
            assert!(
                f.instructions
                    .iter()
                    .any(|i| matches!(i, CallOther(_, RegisterVariant::Constant(_), _))),
                "{:?}",
                f.instructions
            );
    ...
    mod test_condition_form {
        ...
        #[tokio::test]
        async fn a_negated_while_loops_while_false() {
    ...
    mod test_coalesce {
        ...
        async fn a_second_call_result_reads_r0_where_the_first_cannot() {
            ...
            let expected = vec![
                ...
                Call(ustr("g__i____pb__"), ArgList(1)),
                Add(
                    Register(1).as_local(),
                    Register(0).as_local(),
        ...
        async fn a_void_return_call_copies_nothing() {
            ...
            let expected = vec![Call(ustr("f__v____pb__"), ArgList(0)), Ret];
            assert_eq!(
                walker_function_instructions(&mut walker, "create"),
                expected
            );
    ...
    mod test_visit_program {
        ...
        async fn populates_the_instructions() {
            ...
            let expected = vec![
                Call(ustr("init-globals__v____pv__"), ArgList(0)),
                Call(ustr("create__v____pb__"), ArgList(1)),
                Ret,
            ];
    ...
    mod test_inverted_loops {
        ...
        #[tokio::test]
        async fn a_while_jumps_to_its_test_first() {
            let code = "void create() { int i; while (i < 3) { i++; } }";
            let expected = vec![
                Jmp(Address(2)),
                Inc(Register(1).as_local()),
                Jcmp(
        ...
        #[tokio::test]
        async fn a_foreach_loads_the_element_after_its_entry_jump() {
    ...
    mod test_visit_var {
        ...
        async fn test_closure_positional_arguments() {
            ...
            assert_eq!(
                closure.arg_lists[0],
                vec![
                    Arg::Value(RegisterVariant::Constant(Register(0))),
                    Arg::Value(RegisterVariant::Upvalue(Register(0))),
                ]
            );
        }
    }

    mod test_argument_lists {
        use super::*;
        ...
        async fn function_f(code: &str) -> Arc<ProgramFunction> {
            let walker = walk_prog(code).await;
            walker
                .functions
                .values()
        ...
        #[tokio::test]
        async fn a_computed_argument_is_read_from_its_temp() {
            let f =
                function_f("int g(int x) { return x; } int f(int a, int b) { return g(a + b); }")
                    .await;
            assert_eq!(
                f.instructions,
                vec![
                    Instruction::Add(local(1), local(2), local(3)),
                    Instruction::Call(ustr("g__i____pb__i"), ArgList(0)),
                    Instruction::Ret,
                ]
            );
            assert_eq!(f.arg_lists, vec![vec![Arg::Value(local(3))]]);
        }

        #[tokio::test]
        async fn a_named_argument_is_read_from_its_register() {
            let f = function_f("void g(int x) {} void f(int a) { g(a); }").await;
            assert_eq!(
                f.instructions,
                vec![
        ...
        #[tokio::test]
        async fn a_nested_calls_result_is_read_from_r0() {
            let f = function_f(
                "int h(int x) { return x; } int g(int x, int y) { return x + y; } int f(int x) { return g(h(x), 1); }",
            )
            .await;
            assert_eq!(
                f.instructions,
                vec![
                    Instruction::Call(ustr("h__i____pb__i"), ArgList(0)),
                    Instruction::Call(ustr("g__i____pb__i_i"), ArgList(1)),
                    Instruction::Ret,
                ]
            );
            assert_eq!(
                ...
                vec![
                    ...
                    vec![Arg::Value(local(0)), Arg::Value(constant(0))],
                ]
            );
        }

        #[tokio::test]
        async fn every_call_site_owns_a_list() {
            let f = function_f("void g() {} void f() { g(); g(); }").await;
            assert_eq!(
                f.instructions,
                vec![
            ...
            assert_eq!(f.arg_lists, vec![vec![], vec![]]);
        }

        #[tokio::test]
        async fn an_efun_call_lists_its_arguments() {
    ...
    async fn sibling_parents_are_imported_with_shifted_globals() {
        ...
        assert_eq!(
            ...
            vec![
                Call(ustr("init-globals__v__/sibling_a.c__pv__"), ArgList(0)),
                Call(ustr("init-globals__v__/sibling_b.c__pv__"), ArgList(1)),
                Call(ustr("init-globals__v____pv__"), ArgList(2)),
                Call(ustr("create__v__/sibling_b.c__pb__"), ArgList(3)),
                Ret
            ]
        );
    }
    ...
    async fn a_program_reached_through_two_parents_has_one_block() {
        ...
        assert_eq!(
            program.initializer.unwrap().instructions,
            vec![
                Call(ustr("init-globals__v__/grandparent.c__pv__"), ArgList(0)),
                Call(ustr("init-globals__v__/diamond_left.c__pv__"), ArgList(1)),
                Call(ustr("init-globals__v__/diamond_right.c__pv__"), ArgList(2)),
                Call(ustr("init-globals__v____pv__"), ArgList(3)),
                Call(ustr("create__v__/grandparent.c__pb__"), ArgList(4)),
                Ret
            ]
        );
    }
    ...
    mod references {
        ...
        #[tokio::test]
        async fn a_ref_argument_lists_its_cell() {
            let walker =
                walk_prog("void inc(int ref x) { x++; } void f() { int y = 1; inc(ref y); }").await;
            let f = walker.functions.values().find(|f| f.name() == "f").unwrap();
            assert_eq!(
                f.arg_lists,
                vec![vec![Arg::Ref(RegisterVariant::Upvalue(Register(0)))]]
            );
            assert!(
                f.instructions
                    .contains(&NewUpvalue(RegisterVariant::Upvalue(Register(0))))
            );
        }

        #[tokio::test]
        async fn a_ref_of_a_global_lists_the_global() {
            let walker =
                walk_prog("int g; void inc(int ref x) { x++; } void f() { inc(ref g); }").await;
            let f = walker.functions.values().find(|f| f.name() == "f").unwrap();
            assert_eq!(
                f.arg_lists,
                vec![vec![Arg::Ref(RegisterVariant::Global(Register(0)))]]
            );
        }
        ...
        #[tokio::test]
        async fn an_implicit_efun_lvalue_lists_its_cell() {
            let walker = walk_prog(r#"void f() { int n; sscanf("1", "%d", n); }"#).await;
            let f = walker.functions.values().find(|f| f.name() == "f").unwrap();
            assert!(
                matches!(
                    f.arg_lists[0][..],
                    [
                        Arg::Value(_),
                        Arg::Value(_),
                        Arg::Ref(RegisterVariant::Upvalue(_))
                    ]
                ),
                "{:?}",
                f.arg_lists
