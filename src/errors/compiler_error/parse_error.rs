    ...
    E: Display
{
    fn from(err: LalrpopParseError<usize, Token, E>) -> Self {
        match err {
            ...
            LalrpopParseError::UnrecognizedEOF { ref expected, .. } => NewError::new("Unexpected EOF")
            ...
            LalrpopParseError::UnrecognizedToken {
                ...
                .with_note(format_expected(expected)),
            LalrpopParseError::ExtraToken { token: (_start, ref token, _end) } => NewError::new(format!("Extra Token: `{}`", token))
                .with_span(Some(token.span())),
