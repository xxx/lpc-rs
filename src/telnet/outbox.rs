...
/// Pending bytes at which the client is told its output was truncated and
/// everything further is dropped.
...
/// One client's pending bytes and their bound: past [`HIGH_WATER`] text to
/// it is dropped, until what is pending falls below [`LOW_WATER`].
#[derive(Debug)]
...
impl Outbox {
    /// An empty outbox for `connection`.
    ...
    pub(crate) fn fill_from(&mut self, session: &mut Session) {
        ...
        if !self.overflowed && self.out.len() >= HIGH_WATER {
    ...
    pub(crate) fn wrote(&mut self, n: usize) {
        self.out.advance(n);
        if self.overflowed && self.out.len() < LOW_WATER {
...
mod tests {
    ...
    fn under_the_mark_nothing_is_dropped() {
        let (mut outbox, connection) = outbox();
