...
/// A trait for marking objects that contain references that could be garbage-collected.
pub trait GcMark {
    /// Mark indices in the [`Process`]' `upvalues` list that are live.
    ///
    /// # Arguments
    /// * `marked` - A set of indices in the `upvalues` list that are live.
    /// * `processed` - A set of [`UniqueId`]s representing nodes that have already
    ///                 been processed. Used to avoid infinite loops.
    /// * `cell_key` - A [`QCellOwner`] key to use for unlocking [`QCell`]ed data.
    ///
    /// # Returns
...
/// A trait for sweeping objects that contain references that could be garbage-collected.
pub trait GcSweep {
