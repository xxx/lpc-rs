//! Formatted function pointers without invoking them or resolving dynamic targets.
...
/// Describe a pointer's receiver, name, and masked bindings, or return 0 for a dead target.
pub fn function_description<const N: usize>(context: &mut EfunContext<'_, N>) -> Result<()> {
    let ptr = function_arg(context, "function_description", 0)?;
    ...
    if name.len() + suffix_len > MAX_STRING_LENGTH {
        return Err(
            context.runtime_error("function_description: result exceeds maximum string length")
        );
    }
    if !args.is_empty() {
...
mod tests {
    ...
    #[tokio::test]
    async fn descriptions_include_the_receiving_object() {
        let code = indoc! { r#"
            ...
            mixed *create() {
                ...
                return ({
                    ...
                    wildmatch("*->keep_obj_m_no_sell", function_description(f))
                });
            }
        "# };
        assert_eq!(
    ...
    async fn inherited_functions_name_the_receiving_object() {
        let code = indoc! { r#"
            inherit "/grandparent";
            string create() { return function_description(&grandparent_method()); }
        "# };
        assert_eq!(
    ...
    async fn bound_arguments_are_masked_and_holes_are_distinguished() {
        let code = indoc! { r#"
            ...
            mixed *create() {
                return ({
                    function_description(&add(1, , 3)),
                    function_description(&write("secret")),
                    function_description(&add(, , ))
                });
            }
        "# };
        assert_eq!(
    ...
    #[tokio::test]
    async fn efun_names_work_through_a_function_description_pointer() {
        let code = indoc! { r#"
            mixed *create() {
                return map(({ write, &write(), reduce, &function_description() }), &function_description());
            }
        "# };
        assert_eq!(
            strings_of(code).await,
            [
                ...
                r#""/my_file"->write"#,
                r#""/my_file"->reduce"#,
                r#""/my_file"->function_description"#
            ]
        );
    }
    ...
    async fn dynamic_names_do_not_resolve_or_load_the_receiver() {
        let code = indoc! { r#"
            mixed *create() {
                return ({
                    function_description(&->absent()),
                    function_description(&->absent(1)),
                    function_description(papplyv(&->absent(1), ({ "/not_a_real_object" }))),
    ...
    async fn closures_expose_their_generated_unmangled_name() {
        let code =
            r#"string create() { return function_description((: throw("must not run") :)); }"#;
        assert_eq!(
            run_prog(code).await.result().unwrap().as_str(),
    ...
    async fn clone_names_include_the_receivers_clone_id() {
        ...
        let task = vm
            ...
            int create() {
                ...
                return function_description(target->pointer()) == "\"" + file_name(target) + "\"->method";
            }
        "# },
            )
            .await
    ...
    async fn destructed_receivers_return_zero_within_the_transaction() {
        ...
        let task = vm
            ...
            int create() {
                ...
                destruct(target);
                return function_description(local) == 0 && function_description(dynamic) == 0;
            }
        "# },
            )
            .await
    ...
    async fn non_function_arguments_are_runtime_errors() {
        let code = "mixed create() { mixed f = 0; return function_description(f); }";
        let err = try_run_prog(code).await.unwrap_err().to_string();
        assert!(
            err.contains("function_description: int is not a function"),
            "{err}"
        );
    }
    ...
    async fn long_bound_argument_lists_respect_the_string_limit() {
        let code =
            "string create() { return function_description(papplyv(&write(), allocate(8192))); }";
        let err = try_run_prog(code).await.unwrap_err().to_string();
        assert!(
            err.contains("function_description: result exceeds maximum string length"),
            "{err}"
        );
    }
}
