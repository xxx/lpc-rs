...
mod tests {
    ...
    async fn test_environment() {
        ...
        let foo_proc = vm.process_initialize_from_code("/all_inv_foo.c", ob).await.unwrap();
        let bar_proc = vm.process_initialize_from_code("/all_inv_bar.c", ob).await.unwrap();
        let baz_proc = vm.process_initialize_from_code("/all_inv_baz.c", ob).await.unwrap();
