...
pub struct CodegenWalker {
    /// Vector of instructions generated by this walker
...
impl TreeWalker for CodegenWalker {
    ...
    fn visit_switch(&mut self, node: &mut SwitchNode) -> Result<()> {
        ...
        if self.instructions.last().unwrap() != &instruction {
            push_instruction!(self, instruction, node.span);
        }

        // generate all the tests for matching the case statements
        let test_address = self.instructions.len();
        self.labels.insert(test_label, test_address);
