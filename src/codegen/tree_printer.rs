...
impl TreeWalker for TreePrinter {
    fn visit_program(&mut self, program: &ProgramNode) -> Result<(), LPCError> {
        println!("Program");
        self.indent += 2;
    ...
    fn visit_int(&mut self, int: &IntNode) -> Result<(), LPCError> {
        self.println_indented(&format!("Int: {}", int.value));

        Ok(())
    }

    fn visit_binary_op(&mut self, expression: &BinaryOpNode) -> Result<(), LPCError> {
    ...
    fn visit_decl(&mut self, node: &DeclNode) -> Result<(), LPCError> {
