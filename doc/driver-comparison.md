...
| Live recompilation | `recompile_object` updates the prototype and its clones; inheritors need separate updates. [Reference][fl-reload]. | Use `request_object_recompile` and inspect its queued result with `query_object_recompile`. Migration matches declaring source, name and exact type; local callbacks become stale. Active shadow chains and changes to cleanup eligibility are refused. [Reference](efun/request_object_recompile.md). |
