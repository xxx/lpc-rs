...
| Live recompilation | `recompile_object` updates the prototype and its clones; inheritors need separate updates. [Reference][fl-reload]. | Use `request_object_recompile` and inspect its queued result with `query_object_recompile`. Migration matches declaring source, name and exact type; local callbacks become stale. System selectors support state-preserving master/simul-efun upgrades; `request_system_reload` explicitly restarts them with fresh state. Active shadow chains and changes to cleanup eligibility are refused. [Reference](efun/request_object_recompile.md). |
