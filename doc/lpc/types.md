...
* `<type> *` (array)
    ...
    - `a & b` is the members of `a` also found in `b`, in `a`'s order, duplicates kept; `-` removes every occurrence. Members are compared the way `-` compares them: strings by content, objects and nested arrays or mappings by identity.
...
* Casts
    - `(type) expr` asserts that the value is `type` or `0`: the expression's static type
      ...
      checks that the value is an array, not its elements. A destructed object passes every
      cast, the same as `0`.
    ...
    - `(void)` is not a cast target; `(void) expr` is a compile error.
