...
`mixed **unique_array(mixed *arr, string | function f, mixed skip = 0)`
...
`f` is called once per element, with that element as its only argument. A
string `f` names a function called in each element, `element->f()`, and an
element that is not a live object is then dropped without a call. An
...
unique_array(users(), "query_level")
