...
`string function_description(function f)`

Return a formatted description of a function pointer without invoking it.
...
Use `function_name(f)` for just the function name. This efun provides the
formatted output previously returned by `function_name(f)`.

### Examples

```c
// In /example.c, with a local function add(a, b):
function_description(&add());            // "\"/example\"->add"
function_description(&add(10, ));        // "&\"/example\"->add(?,_)"
function_description(&write());          // "\"/example\"->write"
function_description(&write("secret"));  // "&\"/example\"->write(?)"
function_description(&->query_name());   // "->query_name"
```

### See also

`function_name`, `function_object`, `function_info`, `functionp`, `function_exists`, `calling_function`, `papplyv`
