...
permission denied`. A `path` with no source file is put to `compile_object`;
cloning a virtual object clones its blueprint's program under the
blueprint's name. Cloning a resident prototype asks nothing.
