...
before the master is asked or anything is written. A string key must also
be a variable name; any other string key is the runtime error
`save_map: a key must be a variable name, not "<key>"`, raised at the same
point. `file` is an in-game path resolved against the lib root, and `.o` is
always appended.
