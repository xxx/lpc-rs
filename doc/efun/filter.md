...
With a dynamic pointer such as `&->query_wearable_item()`, each array item is
the receiver. A missing method returns `0`, so that item is excluded, just as
with `(: $1->query_wearable_item() :)`.
