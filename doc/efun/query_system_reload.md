...
Return a reload request's status, or 0 if the ID is unknown, expired, or belongs to
`request_object_recompile`. Only the object that requested the reload can query it; other objects receive a permission
...
while the querying task runs. The driver retains the latest 128 completed jobs
across system reload and object recompilation, plus queued and running jobs. IDs are positive and may have gaps from
